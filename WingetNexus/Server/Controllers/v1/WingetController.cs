    ...
    public class WingetController : ControllerBase
    {
        ...
        public IActionResult Information()
        {
            return new JsonResult(new
            {
                Data = new
                {
                    SourceIdentifier = Configuration["Reponame"],
            ...
            foreach (var installer in version.Installers)
            {
                ...
                else
                {
                    var data = new ManifestInstaller()
                    {
                        Architecture = installer.Architecture,
                        InstallerType = installer.InstallerType,
                        InstallerUrl = $"https://localhost:5001/api/v1/Files/{installer.FileName}",//UrlHelpers.UrlFor("Download", "Api", Identifier, version.VersionCode, installer.Architecture, installer.Scope, "")),
                        InstallerSha256 = installer.InstallerSha256,
        ...
        {
            //TODO:  CertificateValidationHelper.ValidateAuthentication(req, log);
